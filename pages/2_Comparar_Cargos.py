...
def carregar_agentes():
    ...
    if not os.path.exists(AGENT_DIR):
        st.error(f"📁 Diretório '{AGENT_DIR}' não encontrado. Execute 'generate_agents.py' primeiro.")
        return pd.DataFrame()
    
    agent_files = [f for f in os.listdir(AGENT_DIR) if f.endswith('.json')]
    if not agent_files:
        st.warning("⚠️ Nenhum agente encontrado. Execute 'generate_agents.py' primeiro.")
        return pd.DataFrame()
...
def calcular_compatibilidade_big_five(pessoa1, pessoa2):
    ...
    scores['extroversao'] = 100 - (abs(diff_extroversao - 3) * 8)  # Ótimo em ~3 pts diferença
...
def analisar_dinamica_time(funcionarios_df):
    ...
    for i in range(len(funcionarios_list)):
        for j in range(i + 1, len(funcionarios_list)):
            ...
            compat, scores = calcular_compatibilidade_big_five(pessoa1, pessoa2)
...
def main():
    ...
    departamentos = ['Todos'] + sorted(df_all['departamento'].unique().tolist())
    dept_selecionado = st.sidebar.selectbox("Filtrar por departamento:", departamentos)
    ...
    with col2:
        ...
        st.metric("🤝 Compatibilidade Média", f"{compat_media:.1f}%", delta_color=cor_delta)
    
    with col3:
        st.metric("📊 Menor Compatibilidade", f"{analise['compatibilidade_min']:.1f}%")
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    ...
                    compat, _ = calcular_compatibilidade_big_five(pessoa1, pessoa2)
                    matriz[i, j] = compat
                else:
        ...
        if analise['analises_detalhadas']:
            ...
            with col1:
                ...
                for analise_par in analises_ordenadas[:3]:
                    ...
                    st.write(f"{cor} **{analise_par['pessoa1']}** ↔ **{analise_par['pessoa2']}**: {compat:.1f}%")
    ...
    df_display_renamed = df_display[colunas_exibir].rename(columns=nomes_colunas)
...
Powered by Claude AI da Anthropic | Análise científica baseada no modelo Big Five
</div>
